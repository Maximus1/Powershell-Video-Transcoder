...
# VMAF-Qualitaetsmessung nach dem Recode.
# Nur sinnvoll wenn das Video neu kodiert wurde (nicht bei reinem Kopieren).
# $vmafStartSec  : Startpunkt der Analyse in Sekunden (Vorspann/Logos ueberspringen).
# $vmafDurationSec: Analysedauer in Sekunden. 300 = 5 Minuten, repraesentativ und schnell genug.
# n_subsample=5  : Nur jeden 5. Frame auswerten – reduziert Laufzeit um ~80% bei kaum Genauigkeitsverlust.
# Schwellenwerte : >= 93 = transparent (nicht vom Original unterscheidbar), < 88 = sichtbarer Qualitaetsverlust.
$vmafStartSec    = 90   # Sekunden ab denen die Analyse beginnt
$vmafDurationSec = 300  # Analysedauer in Sekunden (5 Minuten)

# Automatischer Re-Recode wenn VMAF-Score unter Schwellenwert faellt.
# $vmafAutoRetry  : $true = bei zu niedrigem VMAF automatisch mit CRF-2 neu kodieren.
# $vmafMinScore   : Mindestscore (0-100). Unterschreitung loest Retry aus. Empfehlung: 93.
# $vmafMaxRetries : Maximale Anzahl Wiederholungen. Jeder Retry senkt CRF um 2 (Untergrenze: 14).
$vmafAutoRetry   = $true
$vmafMinScore    = 93
$vmafTolerance   = 1.5  # Retry wird erst ausgeloest wenn Score MEHR als diesen Wert unter $vmafMinScore liegt.
                        # Beispiel: MinScore 93, Toleranz 1.5 → Retry erst unter 91.5.
                        # Werte knapp unter dem Schwellenwert (z.B. 92.8) werden akzeptiert.
$vmafMaxRetries  = 2
...
function Get-MediaInfo {
    ...
    # Helligkeitsanalyse (nur SDR, per FFmpeg signalstats – keine externen Tools noetig).
    # Ergebnis wird als BrightnessAvg gespeichert ($null wenn HDR oder Analyse fehlgeschlagen).
    Write-Host "--------------------------------------------------" -ForegroundColor DarkGray
    $brightnessResult = Get-VideoBrightnessInfo -FilePath $filePath -Duration $mediaInfo.Duration -IsHDR $mediaInfo.HDR
    $mediaInfo.BrightnessAvg = $brightnessResult
    Write-Host "--------------------------------------------------" -ForegroundColor DarkGray
...
function Get-VideoBrightnessInfo {
    # Analysiert die Helligkeit eines SDR-Videos per FFmpeg signalstats-Filter.
    # Gibt den durchschnittlichen YAVG-Wert (0-255) zurueck, oder $null wenn die Analyse
    # nicht sinnvoll ist (HDR-Inhalt) oder fehlgeschlagen ist.
    # Benoetigt keine externen Tools ausser FFmpeg.
    param (
        [string]$FilePath,
        [double]$Duration,
        [bool]$IsHDR
    ...
    # Bei HDR-Material sind SDR-Helligkeitswerte irreführend: HDR-Content sieht als SDR
    # dekodiert immer tiefdunkel aus, auch wenn er es nicht ist. Analyse daher ueberspringen.
    if ($IsHDR) {
        Write-Host "  Helligkeitsanalyse: Übersprungen (HDR-Inhalt erkannt – Analyse wäre irreführend)." -ForegroundColor DarkGray
        return $null
    }

    Write-Host "  Starte Helligkeitsanalyse (9 Segmente à 15s, verteilt über Filmlänge)..." -ForegroundColor Cyan

    try {
        # Analysebereich: 90s bis 82% der Gesamtdauer.
        # 90s: Studiologos und Vorspann sicher ueberspringen.
        # 82%: Schwarzen Abspann großzuegig ausschliessen (robuster als fester Sekundenwert).
        $rangeStart = 90
        $rangeEnd   = [math]::Max([math]::Round($Duration * 0.82), $rangeStart + 60)

        if ($rangeStart -ge $rangeEnd) {
            Write-Host "  Helligkeitsanalyse: Video zu kurz fuer sinnvolle Analyse, übersprungen." -ForegroundColor DarkGray
            return $null
        }

        # 9 Segmente gleichmäßig über den Analysebereich verteilen (ungerade Zahl für saubere Mehrheit).
        # CRF wird nur gesenkt wenn die Mehrheit (5+/9) der Segmente dunkel ist – also das Video
        # ueberwiegend dunkel ist, nicht nur einzelne Szenen.
        # fps=1 pro Segment: 15 Werte x 9 = max. 135 Gesamtsamples.
        # KEIN hwaccel: showinfo ist Software-Filter, GPU-Frames wuerden ignoriert.
        $segmentCount    = 9
        $segmentDuration = 15  # 15s Fenster: Puffer fuer lange GOP-Strukturen (HEVC kann GOP > 10s haben)
        $segmentAvgs     = @()  # Durchschnittswert pro Segment, direkt waehrend der Schleife befuellt
        $allValues       = @()  # Alle Einzelwerte fuer Gesamtstatistik

        for ($i = 0; $i -lt $segmentCount; $i++) {
            $segStart = [math]::Round($rangeStart + ($rangeEnd - $rangeStart - $segmentDuration) * $i / ($segmentCount - 1))

            $startInfo = New-Object System.Diagnostics.ProcessStartInfo
            $startInfo.FileName              = $ffmpegPath
            # Doppelter Seek: -ss vor -i macht Fast-Seek auf den naechsten Keyframe (schnell),
            # -ss 2 nach -i springt dann akkurat 2 Sekunden weiter zum exakten Zielzeitpunkt.
            # So wird der Keyframe-Versatz ausgeglichen ohne den vollen Dekodier-Overhead
            # von reinem accurate seek (der ab Dateistart dekodiert).
            # showinfo schreibt garantiert in stderr; Format: mean:[Y U V] - Y ist Luma 0-255.
            # KEIN hwaccel: showinfo ist Software-Filter, GPU-Frames wuerden ignoriert.
            $fastSeekPos = [math]::Max($segStart - 2, 0)
            $startInfo.Arguments             = "-ss $fastSeekPos -i `"$FilePath`" -ss 2 -t $segmentDuration -vf `"fps=1,showinfo`" -f null NUL"
            $startInfo.RedirectStandardError = $true
            $startInfo.UseShellExecute       = $false
            $startInfo.CreateNoWindow        = $true
            ...
            if ($segValues.Count -gt 0) {
                $segAvg = [math]::Round(($segValues | Measure-Object -Average).Average, 1)
                # Segment-Durchschnitt direkt hier speichern – korrekt, unabhaengig von Frame-Anzahl
                $segmentAvgs += $segAvg
                Write-Host ("  Segment {0}/9 @ {1,5}s : Ø {2}" -f ($i + 1), $segStart, $segAvg) -ForegroundColor DarkGray
            }
            else {
                Write-Host ("  Segment {0}/9 @ {1,5}s : keine Werte (Keyframe-Sprung?)" -f ($i + 1), $segStart) -ForegroundColor DarkGray
            }
        }

        if ($segmentAvgs.Count -eq 0) {
            Write-Host "  Helligkeitsanalyse: Keine Werte ermittelt." -ForegroundColor Yellow
            return $null
        }

        $avg = [math]::Round(($allValues | Measure-Object -Average).Average, 1)
        $min = [math]::Round(($allValues | Measure-Object -Minimum).Minimum, 1)
        $max = [math]::Round(($allValues | Measure-Object -Maximum).Maximum, 1)

        # Mehrheitsprinzip: CRF wird nur gesenkt wenn die Mehrheit der Segmente dunkel ist.
        # 5 von 9 = knappe Mehrheit, daher Schwelle auf 6 gesetzt (klare Mehrheit erforderlich).
        $darkThreshold  = 60
        $majorityNeeded = [math]::Ceiling($segmentAvgs.Count / 2) + 1  # 6 bei 9 Segmenten
        $darkSegments   = ($segmentAvgs | Where-Object { $_ -lt $darkThreshold }).Count

        Write-Host "  Helligkeit (YAVG)  : Ø $avg  |  Min $min  |  Max $max  |  Dunkle Segmente: $darkSegments/$($segmentAvgs.Count)  (Skala 0-255, $($allValues.Count) Samples)" -ForegroundColor DarkCyan

        if ($darkSegments -ge $majorityNeeded) {
            Write-Host "  Einschätzung       : Ueberwiegend dunkles Video ($darkSegments/$($segmentAvgs.Count) Segmente < $darkThreshold) → CRF wird in Set-VolumeGain um 2 gesenkt." -ForegroundColor Yellow
            return $true  # Signal: CRF anpassen
        }
        else {
            Write-Host "  Einschätzung       : Normales Video ($darkSegments/$($segmentAvgs.Count) Segmente < $darkThreshold) → Standard-CRF wird verwendet." -ForegroundColor DarkCyan
            return $false  # Signal: kein Eingriff
        }
    }
    catch {
        Write-Host "  Helligkeitsanalyse: Fehler beim Ausfuehren – $_" -ForegroundColor Red
        return $null
    }
}
#endregion
...
function Set-VolumeGain {
    ...
    try {
        ...
        if ($sourceInfo.Force720p -or $sourceInfo.RecodeRecommended -or $needsReencodeDueToBitDepth -or ($videoCodec -ne $targetVideoCodec) -or ($sourceInfo.FPS -gt 25.1)) {
            ...
            # CRF-Anpassung basierend auf Helligkeitsanalyse.
            # $brightnessAvg ist $true wenn das Video ueberwiegend dunkel ist (5+/9 Segmente < 60),
            # $false wenn nicht, $null wenn HDR oder Analyse fehlgeschlagen.
            if ($brightnessAvg -eq $true) {
                $adjustedQuality = [math]::Max($qualityValue - 2, 14)
                Write-Host "  Helligkeitskorrektur: Video ueberwiegend dunkel → CRF von $qualityValue auf $adjustedQuality gesenkt." -ForegroundColor Yellow
                "`n==== Helligkeitskorrektur: CRF von $qualityValue auf $adjustedQuality gesenkt (ueberwiegend dunkles Video). ====" | Add-Content -LiteralPath $logDatei
                $qualityValue = $adjustedQuality
                # CQP/CQ fuer HW-Encoder entsprechend neu berechnen
                $cqpValue = [math]::Round($qualityValue * 1.2)
                $cqValue  = [math]::Round($qualityValue * 1.2)
            }
            elseif ($brightnessAvg -eq $false) {
                Write-Host "  Helligkeitskorrektur: Nicht erforderlich (normales Video) → Standard-CRF $qualityValue." -ForegroundColor DarkGray
            }
            else {
                Write-Host "  Helligkeitskorrektur: Kein Helligkeitswert verfuegbar → Standard-CRF $qualityValue." -ForegroundColor DarkGray
            }
        ...
        try {
            ...
            # CPU Video-Optionen: qualityValue hier bereits durch Helligkeitskorrektur angepasst (falls zutreffend)
            $newArgs += @(
                "-c:v", "libx265",
                "-pix_fmt", "yuv420p",
...
function Get-VmafScore {
    # Berechnet den VMAF-Score durch direkten Frame-Vergleich von Quelle und Zieldatei.
    # Wird nur aufgerufen wenn das Video tatsaechlich neu kodiert wurde.
    # Gibt den VMAF-Score (0-100) zurueck, oder $null bei Fehler.
    param (
        [string]$SourceFile,
        [string]$OutputFile,
        [double]$StartSec,
    ...
    try {
        $startInfo = New-Object System.Diagnostics.ProcessStartInfo
        $startInfo.FileName = $ffmpegPath
        # Doppelter Fast-Seek auf beide Eingabedateien zum gleichen Zeitpunkt.
        # -t begrenzt die Analysedauer auf $DurationSec Sekunden ab dem Seekpunkt.
        # libvmaf n_subsample=5: jeden 5. Frame auswerten, ~80% schneller als vollstaendige Analyse.
        # KEIN hwaccel: libvmaf ist ein Software-Filter.
        $startInfo.Arguments = "-ss $StartSec -i `"$SourceFile`" -ss $StartSec -i `"$OutputFile`" -t $DurationSec -lavfi `"[0:v][1:v]libvmaf=n_subsample=5`" -f null NUL"
        $startInfo.RedirectStandardError = $true
        $startInfo.UseShellExecute       = $false
        $startInfo.CreateNoWindow        = $true
        ...
        # FFmpeg schreibt den VMAF-Score in stderr: "VMAF score: 96.123456"
        $match = [regex]::Match($output, "VMAF score:\s*([\d\.]+)")
        if (-not $match.Success) {
            Write-Host "  VMAF: Kein Score in FFmpeg-Ausgabe gefunden." -ForegroundColor Yellow
            return $null
        }

        $score = [math]::Round([double]$match.Groups[1].Value, 2)

        if ($score -ge 93) {
            Write-Host ("  VMAF-Score: {0:N2}  → Transparent (nicht vom Original unterscheidbar)" -f $score) -ForegroundColor Green
        }
        elseif ($score -ge 88) {
            Write-Host ("  VMAF-Score: {0:N2}  → Gute Qualitaet, minimale Artefakte moeglich" -f $score) -ForegroundColor Yellow
        }
        else {
            Write-Host ("  VMAF-Score: {0:N2}  → WARNUNG: Sichtbarer Qualitaetsverlust!" -f $score) -ForegroundColor Red
        }

        return $score
    }
    catch {
        Write-Host "  VMAF: Fehler beim Ausfuehren – $_" -ForegroundColor Red
        return $null
    }
}
function Test-OutputFile {
    ...
    # Groesseninformationen fuer den Hauptloop bereitstellen (wird fuer Statistik und VMAF-Retry benoetigt).
    # Gespeichert in $script:lastFileStats damit der Hauptloop nach dem Return darauf zugreifen kann.
    $script:lastFileStats = [PSCustomObject]@{
        Quelle_MB    = [math]::Round($sizeSourceBytes / 1MB, 2)
        Ziel_MB      = [math]::Round($sizeOutputBytes / 1MB, 2)
        Ersparnis_MB = $diffMB
        Ersparnis_Prz = $diffPercent
    }
...
# GetForegroundWindow() holt das Handle des aktuell aktiven Fensters (VSCode) zum Zeitpunkt
# des Scriptstarts. Das ist zuverlaessiger als MainWindowHandle, das im VSCode-Terminal
# IntPtr.Zero zurueckgibt weil der PowerShell-Prozess kein eigenes Top-Level-Fenster hat.
Add-Type -TypeDefinition @"
using System;
using System.Windows.Forms;
using System.Runtime.InteropServices;
public class ForegroundWindowOwner : IWin32Window {
    [DllImport("user32.dll")] public static extern IntPtr GetForegroundWindow();
    public IntPtr Handle { get { return GetForegroundWindow(); } }
}
"@ -ReferencedAssemblies System.Windows.Forms

$dialogOwner = New-Object ForegroundWindowOwner
...
$result = $PickFolder.ShowDialog($dialogOwner)
...
if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
    ...
    foreach ($file in $script:filesnotnorm) {
        ...
        try {
            ...
            # Beteiligte funktionen: Get-MediaInfo, Get-FFmpegOutput, Get-BasicVideoInfo, Get-ColorAndHDRInfo, Get-AudioInfo, Get-InterlaceInfo, Test-IsSeries, Get-RecodeAnalysis, Measure-ExpectedSizeMB, Get-VideoBrightnessInfo
            ...
            if ($ffmpegOutput -match "I:\s*([-\d\.]+)\s*LUFS") {
                ...
                $audiocopyflag = if ($null -ne $encodeResult) { $encodeResult.AudioCopy } else { $true }

                # VMAF-Retry-Schleife: Encode validieren, VMAF messen, bei Unterschreitung
                # des Schwellenwerts automatisch mit CRF-2 neu kodieren ($vmafMaxRetries mal).
                # Sicherheitsnetz: vor jedem Retry wird die aktuelle Ausgabedatei als Backup
                # gesichert. Wenn der neue Encode die Groessenvalidierung nicht besteht (Datei
                # zu gross), wird das Backup wiederhergestellt und mit dessen VMAF weitergemacht.
                $retryCount      = 0
                $crfAdjust       = 0
                $finalVmaf       = $null
                $isOutputOk      = $false
                ...
                do {
                    if ($retryCount -gt 0) {
                        # Aktuelle (gute) Ausgabedatei sichern bevor der neue Encode startet.
                        # So haben wir immer einen Fallback wenn der Retry die Groessengrenze reisst.
                        ...
                        $crfAdjust -= 2
                        $retryCrfTargets = [math]::Max($crfTargets + $crfAdjust, 14)
                        $retryCrfTargetm = [math]::Max($crfTargetm + $crfAdjust, 14)
                        Write-Host "VMAF-Retry $retryCount/$vmafMaxRetries : Neuer Encode mit CRF-Anpassung $crfAdjust (Serien-Basis: $retryCrfTargets, Film-Basis: $retryCrfTargetm)..." -ForegroundColor Yellow
                        "`n==== VMAF-Retry $retryCount : CRF-Anpassung $crfAdjust ====" | Add-Content -LiteralPath $logDatei
                        $encodeResult  = Set-VolumeGain -filePath $file -gain $gain -outputFile $outputFile -audioChannels $sourceInfo.AudioChannels -videoCodec $sourceInfo.VideoCodec -interlaced $sourceInfo.Interlaced -bitDepth $sourceInfo.BitDepth -sourceInfo $sourceInfo -logDatei $logDatei -ffmpegPath $ffmpegPath -videoEncoder $videoEncoder -encoderPreset $encoderPreset -crfTargets $retryCrfTargets -crfTargetm $retryCrfTargetm -amd_quality $amd_quality -Nvidia_quality $Nvidia_quality -targetLoudness $targetLoudness -targetVideoCodec $targetVideoCodec -brightnessAvg $sourceInfo.BrightnessAvg
                        $videocopy     = $encodeResult.VideoCopy
                        $audiocopyflag = $encodeResult.AudioCopy
                    }

                    $isOutputOk = Test-OutputFile -outputFile $outputFile -sourceFile $file -sourceInfo $sourceInfo -targetExtension $targetExtension -logDatei $logDatei -videocopy $videocopy -audiocopy $audiocopyflag

                    if (-not $isOutputOk -and $retryCount -gt 0 -and (Test-Path -LiteralPath $backupFile)) {
                        # Neuer Encode hat Groessenvalidierung nicht bestanden → Backup wiederherstellen.
                        Write-Host "  VMAF-Retry: Neue Datei zu gross oder fehlerhaft → stelle vorherigen Encode wieder her." -ForegroundColor Yellow
                        "`n==== VMAF-Retry: Neue Datei ungueltig, Backup wiederhergestellt. ====" | Add-Content -LiteralPath $logDatei
                        if (Test-Path -LiteralPath $outputFile) {
                            Remove-Item -LiteralPath $outputFile -Force -ErrorAction SilentlyContinue
                        }
                        Move-Item -LiteralPath $backupFile -Destination $outputFile -Force
                        $isOutputOk = $true
                        $finalVmaf  = $bestVmaf  # VMAF des letzten gueltigen Encodes
                        break
                    }

                    # Backup des vorigen Encodes nicht mehr benoetigt
                    ...
                    if ($isOutputOk -and -not $videocopy) {
                        ...
                        # werden still akzeptiert – der Unterschied ist visuell nicht wahrnehmbar.
                        $vmafRetryThreshold = $vmafMinScore - $vmafTolerance
                        if ($vmafAutoRetry -and $null -ne $finalVmaf -and $finalVmaf -lt $vmafRetryThreshold -and $retryCount -lt $vmafMaxRetries) {
                            Write-Host ("  VMAF {0:N2} < {1:N2} (Schwelle {2} - Toleranz {3}) – starte Retry {4}/{5}..." -f $finalVmaf, $vmafRetryThreshold, $vmafMinScore, $vmafTolerance, ($retryCount + 1), $vmafMaxRetries) -ForegroundColor Yellow
                            "`n==== VMAF $finalVmaf unter Retry-Schwelle $vmafRetryThreshold – Retry $($retryCount + 1) ====" | Add-Content -LiteralPath $logDatei
                            $retryCount++
                            continue
                        }
                    }
                    break  # Kein Retry noetig oder nicht moeglich – Schleife verlassen
                } while ($retryCount -le $vmafMaxRetries)

                # Backup aufraemen falls noch vorhanden (z.B. bei Abbruch durch Exception)
                if (Test-Path -LiteralPath $backupFile) {
                    Remove-Item -LiteralPath $backupFile -Force -ErrorAction SilentlyContinue
                }

                # Statistik mit finalem Ergebnis erfassen
                $aktion = if ($videocopy -and $audiocopyflag) {
                    "Kopiert"
                } elseif (-not $videocopy -and -not $audiocopyflag) {
                    "VA-recodiert"
                } elseif (-not $videocopy -and $audiocopyflag) {
                    "V-recodiert"
                } else {
                    "A-recodiert"
                }
                if ($null -ne $script:lastFileStats) {
                    $script:statistics += [PSCustomObject]@{
                        Datei         = [System.IO.Path]::GetFileName($file)
                        Aktion        = $aktion
                        Quelle_MB     = $script:lastFileStats.Quelle_MB
                        Ziel_MB       = $script:lastFileStats.Ziel_MB
                        Ersparnis_MB  = $script:lastFileStats.Ersparnis_MB
                        Ersparnis_Prz = $script:lastFileStats.Ersparnis_Prz
                        VMAF          = $finalVmaf
                    }
                    $script:lastFileStats = $null
                }
    ...
    # Gibt am Ende der gesamten Verarbeitung eine farbkodierte Tabelle mit Summenzeile aus.
    Write-Host "`n--- Statistische Auswertung ---" -ForegroundColor Yellow

    if ($script:statistics.Count -gt 0) {
        # Spaltenbreiten berechnen
        $maxDatei  = [math]::Max(($script:statistics | ForEach-Object { $_.Datei.Length } | Measure-Object -Maximum).Maximum, 7)
        $maxAktion = 12

        $header = "{0,-$maxDatei}  {1,-$maxAktion}  {2,12}  {3,9}  {4,13}  {5,12}  {6,6}" -f "Datei", "Aktion", "Quelle (MB)", "Ziel (MB)", "Ersparnis (MB)", "Ersparnis (%)", "VMAF"
        ...
        foreach ($entry in $script:statistics) {
            $totalQuelle    += $entry.Quelle_MB
            $totalZiel      += $entry.Ziel_MB
            $totalErsparnis += $entry.Ersparnis_MB

            $sizeColor = if ($entry.Ersparnis_MB -gt 0) { "Green" } else { "Red" }
            $sign      = if ($entry.Ersparnis_Prz -ge 0) { "" } else { "+" }

            # VMAF-Wert formatieren und Farbe bestimmen
            if ($null -ne $entry.VMAF) {
                $vmafScores += $entry.VMAF
                $vmafDisplay = "{0:N2}" -f $entry.VMAF
                $vmafColor   = if ($entry.VMAF -ge 93) { "Green" } elseif ($entry.VMAF -ge 88) { "Yellow" } else { "Red" }
            }
            else {
                $vmafDisplay = "-"
                $vmafColor   = "DarkGray"
            ...
            # Zeile ohne VMAF in Grundfarbe ausgeben, VMAF separat farbig anhaengen
            $line = "{0,-$maxDatei}  {1,-$maxAktion}  {2,12:N2}  {3,9:N2}  {4,13:N2}  {5,11}%  " -f `
                $entry.Datei, $entry.Aktion, $entry.Quelle_MB, $entry.Ziel_MB, $entry.Ersparnis_MB, "$sign$($entry.Ersparnis_Prz)"
            Write-Host $line -ForegroundColor $sizeColor -NoNewline
            Write-Host ("{0,6}" -f $vmafDisplay) -ForegroundColor $vmafColor
        }

        Write-Host $separator -ForegroundColor DarkGray

        # Summenzeile
        $totalErsparnisPrz = if ($totalQuelle -gt 0) { [math]::Round(($totalErsparnis / $totalQuelle) * 100, 2) } else { 0 }
        $totalErsparnisMB      = [math]::Round($totalErsparnis, 2)
        $totalErsparnisMB_abs  = [math]::Abs($totalErsparnisMB)
        $totalGB   = [math]::Round($totalErsparnisMB / 1024, 2)
        $sumColor  = if ($totalErsparnis -ge 0) { "Green" } else { "Red" }
        $vmafAvg   = if ($vmafScores.Count -gt 0) { "{0:N2}" -f (($vmafScores | Measure-Object -Average).Average) } else { "-" }
        $vmafAvgColor = if ($vmafScores.Count -gt 0) {
            $avg = ($vmafScores | Measure-Object -Average).Average
            if ($avg -ge 93) { "Green" } elseif ($avg -ge 88) { "Yellow" } else { "Red" }
        } else { "DarkGray" }

        $sumLabel = "GESAMT ($($script:statistics.Count) Dateien)"
        $sumLine = "{0,-$maxDatei}  {1,-$maxAktion}  {2,12:N2}  {3,9:N2}  {4,13:N2}  {5,11}%  " -f `
            $sumLabel, "", [math]::Round($totalQuelle, 2), [math]::Round($totalZiel, 2), $totalErsparnisMB, $totalErsparnisPrz
        Write-Host $sumLine -ForegroundColor $sumColor -NoNewline
        Write-Host ("{0,6}" -f $vmafAvg) -ForegroundColor $vmafAvgColor
        Write-Host ("  → Gesamtersparnis: {0:N2} MB ({1:N2} GB)" -f $totalErsparnisMB_abs, [math]::Abs($totalGB)) -ForegroundColor $sumColor
    }
    else {
        Write-Host "  Keine Dateien verarbeitet." -ForegroundColor DarkGray
    }
